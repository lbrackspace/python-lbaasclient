...
import lbaasclient
import lbaasclient.auth_plugin
from lbaasclient import client
from lbaasclient import exceptions as exc
import lbaasclient.extension
from lbaasclient.openstack.common import strutils
from lbaasclient import utils
from lbaasclient.v1_0 import shell as shell_v1_0

DEFAULT_OS_COMPUTE_API_VERSION = "1.0"
DEFAULT_LBAAS_ENDPOINT_TYPE = 'publicURL'
DEFAULT_LBAAS_SERVICE_TYPE = 'rax:load-balancer'
...
class SecretsHelper(object):
    ...
    def save(self, auth_token, management_url, tenant_id):
        ...
        value = "|".join([str(auth_token),
                          str(management_url),
                          str(tenant_id)])
        keyring.set_password("lbaasclient_auth", self._make_key(), value)
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('lbaasclient_auth', self._make_key())
            if block:
...
class OpenStackComputeShell(object):
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = lbaasclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self):
        for (module_loader, name, _ispkg) in pkgutil.iter_modules():
            if name.endswith('_python_lbaasclient_ext'):
    ...
    def _discover_via_entry_points(self):
        for ep in pkg_resources.iter_entry_points('lbaasclient.extension'):
            name = ep.name
            module = ep.load()
    ...
    def main(self, argv):
        ...
        lbaasclient.auth_plugin.discover_auth_systems()
        ...
        if os_auth_system and os_auth_system != "keystone":
            auth_plugin = lbaasclient.auth_plugin.load_plugin(os_auth_system)
        else:
            auth_plugin = None
        ...
        if not endpoint_type:
            endpoint_type = DEFAULT_LBAAS_ENDPOINT_TYPE

        if not service_type:
            service_type = DEFAULT_LBAAS_SERVICE_TYPE
    ...
    def do_bash_completion(self, _args):
        ...
        lbaas.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
